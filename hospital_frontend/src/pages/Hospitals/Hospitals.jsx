...
export function Hospitals() {
    ...
    useEffect(() => {
        const fetchHospitals = async () => {
            try {
                const response = await api.get('http://127.0.0.1:8000/api/hospitals/', {
        ...
        <div className="min-h-screen bg-gradient-to-br from-teal-400 via-blue-500 to-purple-600 p-6">
            <div className="max-w-4xl mx-auto">
                ...
                <button className="relative inline-flex items-center justify-center p-0.5 mb-6 mx-auto overflow-hidden text-sm font-bold text-white rounded-xl group bg-gradient-to-br from-pink-500 to-yellow-500 group-hover:from-pink-600 group-hover:to-yellow-600 focus:ring-4 focus:outline-none focus:ring-pink-300 transition-all duration-300">
                    <Link to="/hospitals/create" className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-gray-900 rounded-lg group-hover:bg-transparent">
                        Create New Hospital
                    </Link>
                </button>
                        ...
                        <tbody>
                            {hospitals.length > 0 ? (
                                hospitals.map((hospital, index) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 font-medium">{hospital.id}</td>
                                        <td className="px-6 py-4">{hospital.hospital_name}</td>
                                        ...
                                        <td className="flex items-center px-6 py-4 space-x-3">
                                            <Link 
                                                to={`/hospitals/${hospital.id}/update`} 
                                            ...
                                            <Link 
                                                to={`/hospitals/${hospital.id}/delete`} 
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-4 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-900">
                                        No hospitals found
                                    </td>
                                </tr>
                            )}
